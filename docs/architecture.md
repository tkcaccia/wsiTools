...
flowchart TD
  User["User"]
  Desktop["Tauri desktop starter<br/>file selection and R process control"]
  SystemWebview["Platform webview<br/>WKWebView / WebView2 / WebKitGTK"]
  Chromium["Linux Chrome/Chromium app window<br/>optional WebGPU runtime"]
  ...
  WebGPU["Optional browser WebGPU<br/>visible tile compositor"]
  WASM["WebAssembly helper<br/>annotation viewport culling"]
...
| Desktop launcher | File selection, R discovery, startup progress, native Save dialogs and Linux browser-runtime selection | `tools/wsiToolsDesktop` |
...
and waits for the final viewer URL. The supported renderer is the same
browser/OpenSeadragon application used by direct R launches; Tauri does not
...
The optional WebGPU layer receives already decoded visible tiles and composites
them. Image decoding, format support and region reads remain in R-side
OpenSlide, libvips, native CZI or Bio-Formats backends. WebAssembly is a separate
CPU optimization used for bounding-box culling of large annotation collections;
it neither reads image pixels nor performs GPU composition.
...
2. **Optional WebGPU** composites already decoded visible base/channel tiles
   when the selected browser exposes `navigator.gpu` and creates a device.
   OpenSeadragon remains the viewport controller and fallback tile drawer.
3. **WebGL** renders large spatial point layers such as Seurat, Giotto, or
   SpatialExperiment coordinates.
4. **WebAssembly** accelerates CPU-side bounding-box culling for sufficiently
   large static annotation collections, with a JavaScript fallback.
5. **2D canvas** renders editable ROIs, brush previews, trajectories,
   measurements, labels, selection handles, and small vector layers.
6. **HTML panels** render controls, legends, history, logs, jobs, and dialogs.
