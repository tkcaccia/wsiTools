...
test_that("Seurat viewer exposes spot layer and reduction controls", {
  ...
  expect_match(text, "seurat_spots", fixed = TRUE)
	  ...
	  expect_match(text, "seuratSpotDiameterSteps=[1,2,5,10", fixed = TRUE)
	  expect_match(text, "floor=clamp(diameter/8,.35,2.2)", fixed = TRUE)
	  expect_match(text, "function multiViewSpatialMinRadius", fixed = TRUE)
	  expect_match(text, "pointMinRadius=isSpatial?multiViewSpatialMinRadius(layer,pane):2.2", fixed = TRUE)
  ...
  expect_match(text, "request.feature_source=String(options.feature_source)", fixed = TRUE)
	  ...
	  expect_match(text, "seuratPlotResizeHandle", fixed = TRUE)
	  expect_match(text, "Resize dimensionality reduction plot", fixed = TRUE)
	  expect_match(text, "seuratPlotPanelResize", fixed = TRUE)
	  expect_match(text, "seuratPlotResizeObserver.observe(viewport)", fixed = TRUE)
	  expect_match(text, "overflow:hidden;resize:none", fixed = TRUE)
