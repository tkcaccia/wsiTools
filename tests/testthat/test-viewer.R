...
test_that("interactive viewer writes a self-contained HTML file for mock slides", {
  ...
  expect_match(html, "Selected category <select id=\"selectedRoiClassSelect\"", fixed = TRUE)
  expect_match(html, "Apply selected category", fixed = TRUE)
  ...
  expect_match(html, "#workspacePanel{position:fixed;left:12px;top:88px", fixed = TRUE)
