...
test_that("interactive viewer writes a self-contained HTML file for mock slides", {
  ...
  expect_match(html, "deferredToolModeRefresh", fixed = TRUE)
  expect_match(html, "annotationSpotDataAvailable", fixed = TRUE)
  expect_false(grepl(
  ...
  expect_match(html, ".quickToolBar{position:static;display:flex", fixed = TRUE)
  expect_match(html, ".quickToolBar>button,.quickToolBar .quickToolMenu>summary{position:relative;box-sizing:border-box;flex:0 0 34px;width:34px;min-width:34px;max-width:34px;height:32px", fixed = TRUE)
  expect_match(html, ".quickToolBar .quickToolMenu>summary{margin:0;}", fixed = TRUE)
  expect_false(grepl(".quickToolBar button{", html, fixed = TRUE))
  expect_match(
    html,
    "aria-label=\"Viewer menus\"><div class=\"quickToolBar\"",
    fixed = TRUE
  )
  expect_lt(
    regexpr("class=\"quickToolBar\"", html, fixed = TRUE)[1],
  ...
  expect_match(html, "maxPoints=384", fixed = TRUE)
  ...
  expect_match(html, "function proximityPayload(){const scope=trajectoryProjectScope();return {point_source:", fixed = TRUE)
  expect_match(html, "project_image_index:scope.project_image_index", fixed = TRUE)
  ...
  expect_match(html, ">Start new ROI</button>", fixed = TRUE)
  expect_match(html, "function armNewAnnotation", fixed = TRUE)
  expect_match(html, "startNewAnnotation(preferredMode||preferredNewAnnotationMode())", fixed = TRUE)
  expect_match(html, "panelNextSelect.onchange=e=>{clearNextCustom();armNewAnnotation", fixed = TRUE)
  ...
  expect_false(grepl("Set new ROI category", html, fixed = TRUE))
  expect_match(html, "New '+value+' ROI armed", fixed = TRUE)
  ...
  expect_match(html, "denseLayerEditableGroups", fixed = TRUE)
  expect_match(html, "denseGeojsonPromotedIds", fixed = TRUE)
  expect_match(html, "applyBrushMaskToSelectedRoi(groups,target,'wand')", fixed = TRUE)
  expect_match(html, "applyBrushMaskToSelectedRoi(groups,target,'wand_subtract')", fixed = TRUE)
  expect_match(html, "Refining selected tissue annotation", fixed = TRUE)
  ...
  expect_match(html, "#jobSyncIndicator{display:inline-flex;align-items:center;justify-content:flex-start;gap:6px;flex:0 0 146px;width:146px", fixed = TRUE)
  expect_match(html, "#jobSyncIndicator{flex-basis:108px;width:108px", fixed = TRUE)
  expect_match(html, "panel titlePanel", fixed = TRUE)
  expect_match(html, ".titlePanel #jobSyncIndicator{margin-left:auto;}", fixed = TRUE)
  ...
  title_panel_pos <- regexpr("<div class=\"panel titlePanel\">", html, fixed = TRUE)
  nav_dock_pos <- regexpr("<div id=\"navDock\"", html, fixed = TRUE)
  expect_gt(sync_indicator_pos[[1]], title_panel_pos[[1]])
  expect_lt(sync_indicator_pos[[1]], help_menu_pos[[1]])
  ...
  expect_match(html, "miniNavigatorTitle", fixed = TRUE)
  expect_match(html, "miniNavigatorClose", fixed = TRUE)
  expect_match(html, "miniNavigatorToggle", fixed = TRUE)
  expect_match(html, "Navigator preview", fixed = TRUE)
  expect_match(html, "setMiniNavigatorOpen", fixed = TRUE)
  expect_match(html, "syncMiniNavigatorToActiveView", fixed = TRUE)
  expect_match(html, "navigatorActiveMultiViewPane", fixed = TRUE)
  expect_match(html, "navigatorCurrentVisibleBounds", fixed = TRUE)
  ...
  expect_match(html, "Extend the selected tissue annotation", fixed = TRUE)
  ...
  expect_match(html, "class=\"toolMenu quickToolMenu\"", fixed = TRUE)
  expect_match(html, "aria-label=\"Measurement tools\"", fixed = TRUE)
  expect_match(html, "content:attr(aria-label)", fixed = TRUE)
  expect_match(html, "button[aria-label]:hover::after", fixed = TRUE)
  expect_match(html, "summary[aria-label]:hover::before", fixed = TRUE)
  expect_false(grepl("<summary title=\"Distance measurement tools\">Measure</summary>", html, fixed = TRUE))
  ...
  expect_match(html, "id=\"openTrajectoryAnalysis\"", fixed = TRUE)
  expect_match(html, "id=\"trajectoryAnalysisWindow\"", fixed = TRUE)
  expect_match(html, "id=\"trajectoryAnalysisClose\"", fixed = TRUE)
  expect_match(html, "openTrajectoryAnalysisWindow", fixed = TRUE)
  expect_match(html, "bindAnalysisWindows", fixed = TRUE)
  expect_match(html, "toolTrajectory", fixed = TRUE)
  expect_match(html, "finishTrajectory", fixed = TRUE)
  expect_match(html, "if(finish)finish.onclick=()=>finishTrajectory(true)", fixed = TRUE)
  ...
  expect_match(html, "saved on '+(scope.project_section||scope.project_image||'current tissue')+'; trajectory drawing off", fixed = TRUE)
  ...
  expect_match(html, "project_key:scope.project_key", fixed = TRUE)
  expect_match(html, "multiViewLayerItemMatchesPane(t,t,pane)", fixed = TRUE)
  expect_match(html, "layerItemMatchesActiveProject(t,t)", fixed = TRUE)
  expect_match(html, "function seuratGenePayloadScope", fixed = TRUE)
  expect_match(html, "Object.assign(request,scope)", fixed = TRUE)
  expect_match(html, "seuratApplyGeneScope", fixed = TRUE)
  ...
  expect_match(html, "own.forEach(append);current.filter(layer=>layer&&!layer.project_managed).forEach(append)", fixed = TRUE)
  expect_match(html, "layers.splice(0,0,...managed)", fixed = TRUE)
  ...
  expect_match(html, ".bar{position:fixed;left:12px;right:12px;top:10px", fixed = TRUE)
...
test_that("Deep Zoom tile cache is rebuilt when the source image changes", {
  ...
  expect_false(identical(
    wsiTools:::wsi_deepzoom_cache_key(slide1, 16, 1, "png", 90),
    wsiTools:::wsi_deepzoom_cache_key(slide2, 16, 1, "png", 90)
  ))
})
...
test_that("wand smooths boundaries and fills only new tiny artifacts", {
  ...
  expect_match(wand_code, "wandSeedConnected", fixed = TRUE)
  expect_match(wand_code, "wandSmoothClosedPoints", fixed = TRUE)
  expect_match(wand_code, "maxPoints=384", fixed = TRUE)
...
test_that("tiled viewer HTML uses OpenSeadragon with an overlay canvas", {
  ...
  expect_match(html, "event.image||event.data", fixed = TRUE)
  expect_match(html, "name==='tile-drawn'||name==='tile-loaded'", fixed = TRUE)
  expect_match(html, "async function initOpenSeadragon", fixed = TRUE)
  expect_match(html, "await initWebgpuBaseCompositor();try{osdViewer", fixed = TRUE)
  ...
  expect_match(html, "overflow-y:auto;overflow-x:hidden", fixed = TRUE)
  expect_match(html, "toolMenu annotationMenu", fixed = TRUE)
  expect_match(html, "let drawn=0", fixed = TRUE)
  expect_match(html, "if(drawn>0)", fixed = TRUE)
  expect_match(html, "baseImageVisible", fixed = TRUE)
  expect_match(html, "baseImagePayload", fixed = TRUE)
  expect_match(html, "showNavigator:false", fixed = TRUE)
  expect_match(html, "miniNavigatorCanvas", fixed = TRUE)
  ...
  expect_match(html, "multiViewHoverPane", fixed = TRUE)
  expect_match(html, "multiViewBrushCursorPane", fixed = TRUE)
  expect_match(html, "multiViewPointInsidePaneSlide", fixed = TRUE)
  expect_match(html, "multiViewPaneHasEditingContext", fixed = TRUE)
  expect_match(html, "multiViewHoverPane=paneObj", fixed = TRUE)
  expect_match(html, "multiViewHoverPane=null;multiViewLastCanvasPointer=null", fixed = TRUE)
  ...
  expect_match(html, "Click a pane to make it active, then zoom with the mouse wheel, trackpad, or main navigation controls", fixed = TRUE)
  ...
  expect_match(html, "multiViewIndependentTileSource", fixed = TRUE)
  expect_match(html, "multiViewTileSourceForEntry", fixed = TRUE)
  expect_match(html, "Number(index)>=entries.length", fixed = TRUE)
  ...
  expect_false(grepl("multiViewPaneControls", html, fixed = TRUE))
  expect_false(grepl("Zoom in this pane", html, fixed = TRUE))
  expect_false(grepl("Zoom out this pane", html, fixed = TRUE))
  ...
  expect_match(html, "finishProjectDragFallback", fixed = TRUE)
  expect_match(html, "origin:'dragend_fallback'", fixed = TRUE)
  expect_match(html, "projectMultiViewPaneAtPoint", fixed = TRUE)
  expect_match(html, "beginProjectPointerDrag", fixed = TRUE)
  expect_match(html, "origin:'pointer_drag_drop'", fixed = TRUE)
  expect_match(html, "function markProjectDragHandled", fixed = TRUE)
  expect_match(html, "Date.now()-projectDragCompletionAt<1200", fixed = TRUE)
  ...
  expect_match(html, "allow_duplicate:true,origin:'drag_drop'", fixed = TRUE)
  expect_match(html, "duplicate view", fixed = TRUE)
  expect_match(html, "multiViewClearChannelItems(pane)", fixed = TRUE)
  expect_match(html, "pane.blank=false", fixed = TRUE)
  expect_match(html, "pane.openGeneration=generation", fixed = TRUE)
  expect_match(html, "pane.openGeneration!==generation", fixed = TRUE)
  expect_match(html, "paneObj.entry", fixed = TRUE)
  expect_match(html, "Loading '+(entry.label", fixed = TRUE)
