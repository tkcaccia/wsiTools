...
test_that("CellPhenotyper KODAMA MedSAM GeoJSON is exposed in the viewer", {
  ...
  expect_equal(config$kodama$plots[[1L]]$points$colour, c("#E41A1C", "#377EB8"))
  ...
  expect_match(text, "width:min(520px", fixed = TRUE)
  expect_match(text, "resize:both", fixed = TRUE)
  ...
  expect_match(text, "kodamaPointColour", fixed = TRUE)
  ...
  expect_match(text, "Draw a lasso around KODAMA points", fixed = TRUE)
  expect_match(text, "kodamaPointsInCanvasPolygon", fixed = TRUE)
